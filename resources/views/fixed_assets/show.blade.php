    ...
    <div class="bg-white text-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded font-mono font-bold text-blue-700">{{ $asset->asset_code ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Serial / Engine No.:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->serial_engine_no ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Plate No.:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->plate_no ?: 'N/A' }}</p>
            </div>
        </div>
        ...
        <div class="mb-6">
            ...
            <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->asset_description }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->asset_group }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Asset Class:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->asset_class ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Depreciation Type:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->dep_type }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->date_posted?->format('M d, Y') ?? 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Acquisition Date:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->acquisition_date?->format('M d, Y') ?? 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Dep. Start Date:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->dep_start_date?->format('M d, Y') ?? 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Dep. End Date:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->dep_end_date?->format('M d, Y') ?? 'N/A' }}</p>
            </div>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-5 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="text-center">
                    ...
                    <p class="text-lg font-bold text-gray-800">{{ number_format($asset->cost, 2) }}</p>
                </div>
                <div class="text-center">
                ...
                <div class="text-center">
                    ...
                    <p class="text-lg font-bold text-gray-600">{{ number_format($asset->salvage_value, 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->useful_life_years }} yrs ({{ $asset->useful_life_months }} months)</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Remaining Life:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded font-semibold {{ $asset->remaining_life_months > 0 ? 'text-blue-700' : 'text-red-700' }}">{{ $asset->remaining_life_months }} months</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Monthly Depreciation:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ number_format($asset->monthly_depreciation, 2) }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Yearly Depreciation:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ number_format($asset->yearly_depreciation, 2) }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->gl_account ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Depreciation Account:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->depreciation_account ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Cost Center:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->cost_center_name ?: 'N/A' }} {{ $asset->cost_center_code ? '(' . $asset->cost_center_code . ')' : '' }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->assigned_person ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Employee (Accountability):</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->employee_name ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Vendor:</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->vendor_name ?: 'N/A' }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->reference_apv_jv ?: 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-500 mb-1">Reference (Reversal):</label>
                <p class="px-4 py-2 bg-gray-50 border border-gray-200 rounded">{{ $asset->reference_reversal ?: 'N/A' }}</p>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('fixed_assets.index') }}" class="bg-gray-100 text-gray-800 px-6 py-2 rounded hover:bg-gray-200 transition">
        ...
        <div id="dispose-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-2xl w-full max-w-lg mx-4">
                ...
                <form action="{{ route('fixed_assets.dispose', $asset->id) }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    <div>
                        <label class="block font-semibold text-gray-700 mb-2">Asset</label>
                        <p class="px-4 py-2 bg-gray-100 rounded border border-gray-300">{{ $asset->asset_description }}</p>
                    </div>
                    <div>
                        <label class="block font-semibold text-gray-700 mb-2">Disposal Date *</label>
                    ...
                    <div class="flex gap-3 justify-end pt-4 border-t">
                        <button type="button" onclick="document.getElementById('dispose-modal').classList.add('hidden')"
                                class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 font-semibold">
                            Cancel
                        </button>
