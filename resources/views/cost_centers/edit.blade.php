...
<div class="container mx-auto">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
            <h1 class="text-2xl font-bold">EDIT: {{ $costCenter->cost_center_code }}</h1>
            <a href="{{ route('cost_centers.show', $costCenter->id) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded">
        ...
        <form action="{{ route('cost_centers.update', $costCenter->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="cost_center_code" value="{{ old('cost_center_code', $costCenter->cost_center_code) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="dimension" value="{{ old('dimension', $costCenter->dimension) }}"
                        class="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block font-semibold text-gray-300 mb-1">COST CENTER OWNER</label>
                    <input type="text" name="cost_center_owner" value="{{ old('cost_center_owner', $costCenter->cost_center_owner) }}"
                        class="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>
            </div>
            ...
            <div class="bg-gray-900 border border-gray-700 rounded p-4 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="division" value="{{ old('division', $costCenter->division) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="department" value="{{ old('department', $costCenter->department) }}"
                            class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block font-semibold text-gray-300 mb-1">CC</label>
                        <input type="text" name="cc" value="{{ old('cc', $costCenter->cc) }}"
                            class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block font-semibold text-gray-300 mb-1">GENERAL CC</label>
                        <input type="text" name="general_cc" value="{{ old('general_cc', $costCenter->general_cc) }}"
                            class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">
                    </div>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div id="glSelectedList" class="space-y-1">
                        @php $existingGls = is_array($costCenter->gl_accounts) ? $costCenter->gl_accounts : []; @endphp
                        @foreach(old('gl_accounts', $existingGls) as $gl)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <textarea name="lookup" rows="3"
                            class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('lookup', $costCenter->lookup) }}</textarea>
                    </div>
                    <div>
                        <label class="block font-semibold text-gray-300 mb-1">OPEX MAPPING</label>
                        <textarea name="opex_mapping" rows="3"
                            class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('opex_mapping', $costCenter->opex_mapping) }}</textarea>
                    </div>
                </div>
            </div>

            <div class="flex justify-end gap-3">
                <a href="{{ route('cost_centers.show', $costCenter->id) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded">Cancel</a>
                <button type="submit" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-2 rounded">
                    <i class="fas fa-save mr-1"></i> Update Cost Center
                </button>
            </div>
        </form>
    </div>
...
document.querySelectorAll('.gl-item input[type=hidden]').forEach(inp => {
    try { const g = JSON.parse(inp.value); if (g.id) selectedGlIds.add(parseInt(g.id)); } catch(e){}
});
...
function removeGl(btn) {
    ...
    try { const g = JSON.parse(item.querySelector('input').value); selectedGlIds.delete(parseInt(g.id)); } catch(e){}
    item.remove();
}
</script>
@endsection
