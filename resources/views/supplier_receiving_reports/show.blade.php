...
<div class="container mx-auto">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="space-y-3">
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">DATE:</label>
                    <p class="text-gray-200">{{ $report->report_date->format('M d, Y') }}</p>
                </div>
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">SUPPLY:</label>
                    <p class="text-gray-200">{{ $report->supplier_name }}</p>
                </div>
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">CV NO:</label>
                    <p class="text-gray-200">{{ $report->cv_no ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">CREATED BY:</label>
                    <p class="text-gray-200">{{ $report->creator->name ?? 'N/A' }} - {{ $report->created_at->format('M d, Y h:i A') }}</p>
                </div>
            </div>
            <div class="space-y-3">
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">STORAGE:</label>
                    <p class="text-gray-200">{{ $report->storage ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">PO NO:</label>
                    <p class="text-gray-200">{{ $report->po_no ?? 'N/A' }}</p>
                </div>
                <div>
                    <label class="block font-semibold text-gray-300 text-sm">TYPE:</label>
        ...
        <!-- Items Table -->
        <div class="mb-6">
            <h3 class="font-semibold text-lg text-gray-300 mb-3">ITEMS</h3>
        ...
        @if($report->note)
            <div class="mb-6">
                <label class="block font-semibold text-gray-300 text-sm mb-1">NOTE:</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            <div>
                <label class="block font-semibold text-gray-300 text-sm">PREPARED BY:</label>
                <p class="text-gray-200">{{ $report->prepared_by ?? 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-300 text-sm">CHECKED BY:</label>
                <p class="text-gray-200">{{ $report->checked_by ?? 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-300 text-sm">RECEIVED BY:</label>
                <p class="text-gray-200">{{ $report->received_by ?? 'N/A' }}</p>
            </div>
            <div>
                <label class="block font-semibold text-gray-300 text-sm">VERIFIED BY:</label>
