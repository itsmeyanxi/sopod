    ...
    <div class="bg-white text-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-4 gap-3 mb-6">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center">
                <p class="text-xs text-gray-600 font-semibold">Paid</p>
                <p class="text-xl font-bold text-gray-700">{{ number_format($summary['paid']) }}</p>
            </div>
        ...
        <!-- Filters -->
        <form method="GET" action="{{ route('loans.index') }}" class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search loan #, lender, purpose..." class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm">
                <select name="loan_type" class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('loans.index') }}" class="bg-gray-200 text-gray-700 px-3 py-2 rounded text-sm hover:bg-gray-300"><i class="fas fa-times"></i></a>
                </div>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm border-collapse">
                <thead class="bg-gray-50 text-gray-500 uppercase text-xs">
                    <tr>
                        <th class="border border-gray-200 px-3 py-3 text-left">Loan No.</th>
                        <th class="border border-gray-200 px-3 py-3 text-center">Date</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">Lender</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">Type</th>
                        <th class="border border-gray-200 px-3 py-3 text-right">Principal</th>
                        <th class="border border-gray-200 px-3 py-3 text-right">Outstanding</th>
                        <th class="border border-gray-200 px-3 py-3 text-center">Rate %</th>
                        <th class="border border-gray-200 px-3 py-3 text-center">Maturity</th>
                ...
                <tbody>
                    ...
                    @endphp
                    <tr class="hover:bg-gray-50 border-b border-gray-100 {{ $isOverdue ? 'bg-red-50' : '' }}">
                        <td class="border border-gray-200 px-3 py-2">
                            <a href="{{ route('loans.show', $loan->id) }}" class="text-blue-700 hover:underline font-mono font-bold text-xs">{{ $loan->loan_no }}</a>
                        </td>
                        <td class="border border-gray-200 px-3 py-2 text-center text-xs">{{ $loan->loan_date->format('M d, Y') }}</td>
                        <td class="border border-gray-200 px-3 py-2 font-semibold">{{ $loan->lender_name }}</td>
                        ...
                        <td class="border border-gray-200 px-3 py-2 text-right">{{ number_format($loan->principal_amount, 2) }}</td>
                        <td class="border border-gray-200 px-3 py-2 text-right font-semibold {{ $isOverdue ? 'text-red-600' : '' }}">{{ number_format($loan->outstanding_principal, 2) }}</td>
                        <td class="border border-gray-200 px-3 py-2 text-center text-xs">{{ number_format($loan->interest_rate, 2) }}%</td>
                        <td class="border border-gray-200 px-3 py-2 text-center text-xs {{ $isOverdue ? 'text-red-600 font-semibold' : '' }}">
