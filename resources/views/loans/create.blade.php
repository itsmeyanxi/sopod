...
<div class="container mx-auto">
    <div class="bg-white text-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
            <h1 class="text-2xl font-bold text-gray-800">NEW LOAN</h1>
        ...
        <form method="POST" action="{{ route('loans.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                    ...
                    <input type="text" value="{{ $nextLoanNo }}" readonly class="w-full bg-gray-100 border border-gray-300 rounded px-3 py-2 text-sm font-mono text-gray-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="loan_type" required class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm @error('loan_type') border-red-400 @enderror">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input type="text" name="lender_name" value="{{ old('lender_name') }}" required
                           class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm @error('lender_name') border-red-400 @enderror"
            ...
            <div class="mb-6">
                <label class="block text-sm font-semibold text-gray-600 mb-1">Purpose / Description</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div>
                    ...
                    <input type="number" name="principal_amount" value="{{ old('principal_amount') }}" required step="0.01" min="0.01"
                           class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm @error('principal_amount') border-red-400 @enderror"
                           placeholder="0.00">
                    @error('principal_amount')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-600 mb-1">Interest Rate (% per annum) <span class="text-red-500">*</span></label>
                    <input type="number" name="interest_rate" value="{{ old('interest_rate') }}" required step="0.0001" min="0"
                           class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm @error('interest_rate') border-red-400 @enderror"
                           placeholder="e.g. 7.5">
                    @error('interest_rate')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-600 mb-1">Loan Term (Months) <span class="text-red-500">*</span></label>
                    <input type="number" name="loan_term_months" value="{{ old('loan_term_months') }}" required min="1"
                           class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm @error('loan_term_months') border-red-400 @enderror"
                           placeholder="e.g. 60">
                ...
                <div>
                    ...
                    <input type="date" name="maturity_date" value="{{ old('maturity_date') }}" required
                           class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm @error('maturity_date') border-red-400 @enderror">
            ...
            <!-- GL Accounts -->
            <h2 class="text-base font-semibold text-gray-600 mb-3 border-b pb-2">GL Account Mapping <span class="text-gray-400 text-xs font-normal">(optional)</span></h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="approved_by" value="{{ old('approved_by') }}"
                           class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm" placeholder="Approver name">
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <textarea name="terms_conditions" rows="3"
                              class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm">{{ old('terms_conditions') }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-600 mb-1">Remarks</label>
                    <textarea name="remarks" rows="3"
                              class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm">{{ old('remarks') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('loans.index') }}" class="bg-gray-200 text-gray-700 px-6 py-2 rounded hover:bg-gray-300 text-sm">Cancel</a>
            </div>
        </form>
    </div>
</div>
