        ...
        <div class="overflow-x-auto">
            <table class="w-full border-collapse border border-gray-200">
                <thead class="bg-gray-100 text-gray-500 uppercase text-xs">
                    <tr>
                        <th class="border border-gray-200 px-3 py-3 text-left">#</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">ITEM CODE</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">ITEM DESCRIPTION</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">GROUP</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">BRAND</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">UoM</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">TRADING UoM</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">CONVERSION</th>
                        <th class="border border-gray-200 px-3 py-3 text-center">STATUS</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">SUPPLIER</th>
                        <th class="border border-gray-200 px-3 py-3 text-center">ACTION</th>
                    </tr>
                </thead>
                <tbody class="text-gray-600">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50">
                            <td class="border border-gray-200 px-3 py-2 text-gray-500">{{ $items->firstItem() + $loop->index }}</td>
                            <td class="border border-gray-200 px-3 py-2">
                                <form action="{{ route('non_trade_items.update', $item->id) }}" method="POST" class="flex items-center gap-1">
                                    ...
                                    <input type="text" name="item_code" value="{{ $item->item_code }}"
                                        class="w-28 px-2 py-1 bg-gray-50 border border-gray-200 rounded text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                                        placeholder="—">
                            ...
                            <td class="border border-gray-200 px-3 py-2">{{ $item->group ?? '-' }}</td>
                            <td class="border border-gray-200 px-3 py-2">{{ $item->brand ?? '-' }}</td>
                            <td class="border border-gray-200 px-3 py-2">{{ $item->unit ?? '-' }}</td>
                            <td class="border border-gray-200 px-3 py-2">{{ $item->trading_uom ?? '-' }}</td>
                            <td class="border border-gray-200 px-3 py-2">{{ $item->conversion ?? '-' }}</td>
...
<div id="importModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-lg p-6 w-full max-w-lg mx-4">
        <h3 class="text-lg font-bold text-gray-800 mb-2">Import Items from CSV/Excel</h3>
        ...
        <form action="{{ route('non_trade_items.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label class="block text-gray-600 mb-2">Select CSV or Excel File:</label>
    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('non_trade_items.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    <label class="block text-gray-600 text-sm mb-1">Group</label>
                    <input type="text" name="group" maxlength="200"
                        class="w-full bg-gray-50 border border-gray-200 rounded px-3 py-2 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="e.g. Office Supplies">
                </div>
                <div>
                    <label class="block text-gray-600 text-sm mb-1">Brand</label>
                    <input type="text" name="brand" maxlength="200"
                        class="w-full bg-gray-50 border border-gray-200 rounded px-3 py-2 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="e.g. Microsoft">
                </div>
                <div>
                    <label class="block text-gray-600 text-sm mb-1">UoM</label>
                    <input type="text" name="unit" maxlength="100"
                        class="w-full bg-gray-50 border border-gray-200 rounded px-3 py-2 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="e.g. pcs, box, kg">
                </div>
                <div>
                    <label class="block text-gray-600 text-sm mb-1">Trading UoM</label>
                    <input type="text" name="trading_uom" maxlength="100"
                        class="w-full bg-gray-50 border border-gray-200 rounded px-3 py-2 text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
                        placeholder="e.g. pack, carton">
                </div>
                <div>
                    <label class="block text-gray-600 text-sm mb-1">Conversion</label>
                ...
                <div class="md:col-span-2">
                    <label class="block text-gray-600 text-sm mb-1">Supplier</label>
