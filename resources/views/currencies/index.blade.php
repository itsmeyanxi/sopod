...
<div class="container mx-auto">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
        ...
        <div class="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
            ...
            <p class="text-gray-300 text-sm">Rates are used to convert foreign currencies to Philippine Peso (PHP) in Purchase Orders.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-900 border border-gray-700 rounded-lg p-5">
                ...
                <div class="mb-3">
                    <div class="text-gray-300 text-xs mb-1">Current Rate (1 {{ $currency->code }} = ? PHP)</div>
                ...
                <form action="{{ route('currencies.update', $currency->id) }}" method="POST" class="flex gap-2 items-end">
                    ...
                    @method('PUT')
                    <div class="flex-1">
                        <label class="block text-gray-300 text-xs mb-1">New Rate (₱ per 1 {{ $currency->code }})</label>
                        <input type="number"
                               name="rate_to_php"
                               step="0.0001"
                               min="0.0001"
                               value="{{ number_format($currency->rate_to_php, 4, '.', '') }}"
                               class="w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"
                               required>
                    </div>
                    <button type="submit"
                ...
                <table class="w-full border-collapse text-sm">
                    ...
                    <tbody class="text-gray-200">
                        ...
                        <tr class="hover:bg-gray-700/30">
                            ...
                            <td class="border border-gray-700 px-4 py-2 text-gray-300 text-xs">{{ $currency->updated_at->format('M d, Y h:i A') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
