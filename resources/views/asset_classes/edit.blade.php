...
<div class="container mx-auto">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6 max-w-2xl mx-auto">
        ...
        <form action="{{ route('asset_classes.update', $assetClass->id) }}" method="POST">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                @include('partials.gl_account_selector', ['field' => 'gl_account', 'label' => 'GL Account', 'uid' => 'ac_gl', 'value' => old('gl_account', $assetClass->gl_account), 'glAccounts' => $glAccounts])
                @include('partials.gl_account_selector', ['field' => 'depreciation_account', 'label' => 'Depreciation Account', 'uid' => 'ac_dep', 'value' => old('depreciation_account', $assetClass->depreciation_account), 'glAccounts' => $glAccounts])
            </div>
