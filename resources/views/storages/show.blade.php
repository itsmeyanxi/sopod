...
<div class="container mx-auto">
    <div class="bg-white text-gray-800 rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">{{ $storage->storage_name }}</h1>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('storages.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded transition">
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-gray-800 font-semibold">{{ $storage->warehouse?->warehouse_name ?? 'Not Assigned' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 text-sm">Location</p>
                        <p class="text-gray-800 font-semibold">{{ $storage->location ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 text-sm">Temperature Controlled</p>
                        <span class="inline-block px-2 py-1 rounded text-xs {{ $storage->temperature_controlled === 'Yes' ? 'bg-blue-100/50 text-blue-700' : 'bg-gray-100 text-gray-500' }}">
                            {{ $storage->temperature_controlled }}
                        </span>
                    </div>
                    <div>
                        <p class="text-gray-500 text-sm">Capacity</p>
                        <p class="text-gray-800 font-semibold">{{ $storage->capacity ? $storage->capacity . ' Liters' : '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-gray-800 font-semibold">{{ $storage->creator?->name ?? 'System' }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 text-sm">Created At</p>
                        <p class="text-gray-800 font-semibold">{{ $storage->created_at->format('M d, Y H:i') }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500 text-sm">Last Updated</p>
                        <p class="text-gray-800 font-semibold">{{ $storage->updated_at->format('M d, Y H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
