...
.bank-card:hover { border-color:#3b82f6; box-shadow:0 2px 8px rgba(59,130,246,.12); transform:translateY(-1px); }
.bank-icon {
...
.bank-card .acct-num { font-size:.72rem; color:#6b7280; font-family:monospace; }
.bank-card .bank-label { font-size:.88rem; font-weight:700; color:#1f2937; }
.bank-card .balance { font-size:1.05rem; font-weight:800; color:#111827; text-align:right; margin-left:auto; }
.bank-card .balance-date { font-size:.62rem; color:#9ca3af; text-align:right; }
...
.bank-table tbody tr { border-bottom:1px solid #f3f4f6; transition:background .1s; cursor:pointer; }
.bank-table tbody tr:hover { background:#f0f7ff; }
.bank-table tbody td { padding:.55rem .75rem; color:#374151; vertical-align:middle; }
.bank-table tbody td.r { text-align:right; font-variant-numeric:tabular-nums; }
.bank-table tfoot td { padding:.6rem .75rem; font-weight:700; background:#f9fafb; border-top:2px solid #e5e7eb; }
</style>
...
<!-- TOTAL BALANCE CARD -->
<div class="stat-card mb-5" style="border-color:{{ $cur==='PHP' ? '#3b82f6' : '#22c55e' }};background:linear-gradient(135deg,{{ $cur==='PHP' ? '#eff6ff' : '#f0fdf4' }},#fff);">
    <div class="stat-lbl" style="color:{{ $cur==='PHP' ? '#3b82f6' : '#22c55e' }};">Total {{ $cur === 'PHP' ? 'Peso' : 'Dollar' }} Cash Balance</div>
    <div class="stat-val" style="color:{{ $cur==='PHP' ? '#1e40af' : '#166534' }};">
        ...
        <table class="bank-table">
            ...
            @foreach($accounts as $i => $acct)
                <tr onclick="window.location='{{ route('treasury.bank.show', $acct->id) }}'">
                    ...
                    <td class="font-semibold text-blue-700">{{ $acct->short_name ?: '—' }}</td>
                    <td>
                        @if($acct->account_type)
                        <span class="inline-block px-2 py-0.5 rounded text-xs font-bold {{ $acct->account_type === 'SA' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700' }}">
                            {{ $acct->account_type }}
                        </span>
                        @else — @endif
                    </td>
