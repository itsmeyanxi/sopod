                    ...
                    <div>
                        ...
                        <form action="{{ route('receiving-reports.updateDr', $receivingReport->id) }}" method="POST" id="drEditForm" class="hidden mt-1 flex items-center gap-2">
                            ...
                            <input type="text" name="delivery_batch" value="{{ $receivingReport->delivery_batch }}"
                                class="bg-white border border-gray-300 rounded px-2 py-1 text-sm text-gray-800 focus:ring-2 focus:ring-blue-500 focus:outline-none w-40">
            ...
            <div class="bg-white/50 rounded-lg p-6 border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <p class="text-gray-700">{{ $receivingReport->received_date ? $receivingReport->received_date->format('M d, Y h:i A') : '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Request Delivery Date</label>
                        <p class="text-gray-700">{{ $receivingReport->request_delivery_date ? \Carbon\Carbon::parse($receivingReport->request_delivery_date)->format('M d, Y') : '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Received By</label>
                        <p class="text-gray-700">{{ $receivingReport->received_by ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Plate No</label>
                        <p class="text-gray-700">{{ $receivingReport->plate_no ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Sales Invoice No</label>
                        <p class="text-gray-700">{{ $receivingReport->sales_invoice_no ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Created By</label>
                        <p class="text-gray-700">{{ $receivingReport->created_by ?? '—' }}</p>
                    </div>
                </div>
                ...
                <div class="mt-4 pt-4 border-t border-gray-200">
                    ...
                    <p class="text-gray-700 bg-gray-50/50 p-3 rounded">{{ $receivingReport->additional_instructions }}</p>
                </div>
                @endif
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-gray-700">{{ $adj->customer_name }}</td>
                                <td class="px-4 py-3 font-mono text-xs text-gray-600">{{ $adj->dr_no }}</td>
                                ...
                                <td class="px-4 py-3 text-center text-gray-600">{{ $adj->transaction_date->format('M d, Y') }}</td>
