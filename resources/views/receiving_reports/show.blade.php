        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex items-center gap-3">
                <span class="px-4 py-2 rounded-lg text-sm font-semibold
                    {{ $receivingReport->status == 'Received' ? 'bg-green-900/30 text-green-400 border border-green-700' : '' }}
                    {{ $receivingReport->status == 'Processing' ? 'bg-yellow-900/30 text-yellow-400 border border-yellow-700' : '' }}
                    {{ $receivingReport->status == 'Completed' ? 'bg-blue-900/30 text-blue-400 border border-blue-700' : '' }}">
                    {{ $receivingReport->status }}
                </span>
                <span class="px-4 py-2 rounded-lg text-sm font-semibold bg-orange-900/30 text-orange-400 border border-orange-700">
            ...
            <div class="bg-white/50 rounded-lg p-6 border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Sales Order Number</label>
                        <p class="text-gray-200 font-mono">{{ $receivingReport->sales_order_number }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Customer Code</label>
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">PO Number</label>
                        <p class="text-gray-200">{{ $receivingReport->po_number ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Sales Representative</label>
                        <p class="text-gray-200">{{ $receivingReport->sales_representative ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Sales Executive</label>
                        <p class="text-gray-200">{{ $receivingReport->sales_executive ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Delivery Batch</label>
                        <p class="text-gray-200">{{ $receivingReport->delivery_batch ?? '—' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/50 rounded-lg p-6 border border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Request Delivery Date</label>
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Received By</label>
                        <p class="text-gray-200">{{ $receivingReport->received_by ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Plate No</label>
                        <p class="text-gray-200">{{ $receivingReport->plate_no ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Sales Invoice No</label>
                        <p class="text-gray-200">{{ $receivingReport->sales_invoice_no ?? '—' }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Created By</label>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-700">
                            ...
                            <tr class="text-gray-500 hover:bg-white/50">
                                ...
                                <td class="px-4 py-3 text-right text-blue-400">{{ number_format($item->original_quantity ?? 0, 2) }}</td>
                                <td class="px-4 py-3 text-right font-semibold text-green-400">{{ number_format($item->quantity, 2) }}</td>
            ...
            <div class="flex justify-between text-xs text-gray-500 pt-4 border-t border-gray-200">
                ...
                <div>
                    ...
                    <span class="text-gray-400">{{ $receivingReport->updated_at->format('M d, Y h:i A') }}</span>
                </div>
                @endif
            </div>
        </div>
