...
<div class="container mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
            <h1 class="text-2xl font-bold text-gray-800">AP Aging Report</h1>
        ...
        @foreach($bucketLabels as $key => $info)
            ...
            <div class="mb-6">
                <h3 class="text-md font-semibold text-gray-700 mb-2">{{ $info['label'] }} ({{ number_format($buckets[$key]['total'], 2) }})</h3>
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse text-sm">
                        <thead class="bg-gray-50 text-gray-500 uppercase text-xs">
                            <tr>
                                <th class="border border-gray-200 px-3 py-2 text-left">APV No.</th>
                                <th class="border border-gray-200 px-3 py-2 text-left">Vendor</th>
                                <th class="border border-gray-200 px-3 py-2 text-center">Due Date</th>
                                <th class="border border-gray-200 px-3 py-2 text-center">Days Overdue</th>
                                <th class="border border-gray-200 px-3 py-2 text-right">Amount</th>
                        ...
                        <tbody>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="border border-gray-200 px-3 py-2 font-semibold text-blue-600">{{ $inv->apv_no }}</td>
                                <td class="border border-gray-200 px-3 py-2 text-gray-700">{{ $inv->vendor_name }}</td>
                                <td class="border border-gray-200 px-3 py-2 text-center text-gray-700">{{ $inv->due_date ? \Carbon\Carbon::parse($inv->due_date)->format('M d, Y') : '-' }}</td>
                                <td class="border border-gray-200 px-3 py-2 text-center {{ $inv->days_overdue > 0 ? 'text-red-600 font-semibold' : 'text-green-600' }}">{{ $inv->days_overdue }}</td>
                                <td class="border border-gray-200 px-3 py-2 text-right font-semibold text-gray-800">{{ number_format($inv->balance, 2) }}</td>
