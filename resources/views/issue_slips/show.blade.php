...
<div class="container mx-auto">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
        ...
        <div class="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
            <h1 class="text-2xl font-bold text-white">ISSUE SLIP</h1>
            <div class="flex gap-3">
                ...
                <a href="{{ route('issue_slips.index') }}" class="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="space-y-2">
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Issue Slip No:</span> <span class="text-white font-bold">{{ $issueSlip->issue_slip_number }}</span></p>
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Date:</span> {{ $issueSlip->date->format('M d, Y') }}</p>
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Origin:</span> {{ $issueSlip->origin ?? 'N/A' }}</p>
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Branch:</span> {{ $issueSlip->branch ?? optional($issueSlip->salesOrder)->branch ?? optional(optional($issueSlip->salesOrder)->customer)->branch ?? 'N/A' }}</p>
            </div>
            <div class="space-y-2">
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Sales Order:</span>
                ...
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Destination:</span> {{ $issueSlip->destination ?? $issueSlip->customer_name ?? 'N/A' }}</p>
                <p class="text-gray-300"><span class="font-semibold text-gray-300">Created By:</span> {{ $issueSlip->creator->name ?? 'N/A' }}</p>
            </div>
        </div>

        @if($issueSlip->salesOrder && $issueSlip->salesOrder->additional_instructions)
        <div class="mb-6">
            <p class="text-gray-300"><span class="font-semibold text-gray-300">Delivery Instructions:</span> {{ $issueSlip->salesOrder->additional_instructions }}</p>
        </div>
        @endif

        @if($issueSlip->remarks)
        <div class="mb-6">
            <p class="text-gray-300"><span class="font-semibold text-gray-300">Issue Slip Remarks:</span> {{ $issueSlip->remarks }}</p>
        </div>
        @endif
                ...
                <table class="w-full border-collapse border border-gray-700">
                    <thead class="bg-red-700 text-white">
                        <tr>
                            ...
                            <th class="border border-gray-700 px-2 py-2">CATEGORY</th>
        ...
        <div class="mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-gray-300"><span class="font-semibold text-gray-300">Issued By:</span> {{ $issueSlip->issued_by ?? 'Not set' }}</p>
                </div>
                <div>
                    <p class="text-gray-300"><span class="font-semibold text-gray-300">Transport:</span> {{ $issueSlip->transport ?? 'Not set' }}</p>
                </div>
                <div>
                    <p class="text-gray-300"><span class="font-semibold text-gray-300">Service Providers Checker:</span> {{ $issueSlip->service_providers_checker ?? 'Not set' }}</p>
                </div>
                <div>
                    <p class="text-gray-300"><span class="font-semibold text-gray-300">Received By:</span> {{ $issueSlip->received_by ?? 'Not set' }}</p>
                </div>
            </div>
        </div>
