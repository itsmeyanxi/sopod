            ...
            <form method="GET" action="{{ route('soa.index') }}" class="flex items-center gap-3">
                <div class="flex-1">
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search by customer name or code..."
                        class="w-full bg-gray-50 border border-gray-200 rounded-md px-4 py-2.5 text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-md font-medium transition">
                    <i class="fas fa-search mr-1"></i> Search
                </button>
                @if($search)
                <a href="{{ route('soa.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2.5 rounded-md font-medium transition">Clear</a>
                @endif
            </form>
        </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100 hover:bg-red-50 transition">
                            <td class="px-4 py-3 text-gray-600 font-mono text-xs">{{ $customer->customer_code }}</td>
                            <td class="px-4 py-3 font-semibold text-gray-800">{{ $customer->client_name }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $customer->branch ?? '—' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $customer->sales_executive ?? '—' }}</td>
                            <td class="px-4 py-3 text-center text-gray-600">{{ $customer->terms ?? '—' }}</td>
