...
<div class="p-6 bg-gray-50 min-h-screen">
    <div class="max-w-6xl mx-auto">
        <div class="mb-4 flex justify-between items-center">
            <a href="{{ route('soa.index') }}" class="text-sm text-gray-500 hover:text-gray-700">
        ...
        <!-- SOA Card -->
        <div class="bg-white rounded-lg shadow-lg overflow-hidden" id="soa_card">
            <!-- Header -->
            <div class="px-8 pt-6 pb-4">
                ...
                <div class="grid grid-cols-2 gap-6 mt-6">
                    <div>
                        ...
                        <p class="text-lg font-bold text-gray-800">{{ $customerName }}</p>
                    ...
                    <div class="text-right space-y-1">
                        <div class="flex justify-end gap-4">
                            ...
                            <span class="text-sm text-gray-800">{{ $today->format('F d, Y') }}</span>
                        </div>
                        <div class="flex justify-end gap-4">
                            <span class="text-xs text-gray-500 font-semibold">Statement Period</span>
                            <span class="text-sm text-gray-800">{{ $today->format('F Y') }}</span>
                        </div>
                        @if($branch)
                        <div class="flex justify-end gap-4">
                            <span class="text-xs text-gray-500 font-semibold">Branch</span>
                            <span class="text-sm text-gray-800">{{ $branch }}</span>
                        </div>
                        @endif
                        @if($terms)
                        <div class="flex justify-end gap-4">
                            <span class="text-xs text-gray-500 font-semibold">Terms</span>
                            <span class="text-sm text-gray-800">{{ $terms }} days</span>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm border-collapse">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $idx % 2 === 1 ? 'bg-red-50' : 'bg-white' }} border-b border-gray-100">
                                <td class="px-3 py-2 text-center text-gray-700">{{ $row->invoice_date ? \Carbon\Carbon::parse($row->invoice_date)->format('n/j/Y') : '—' }}</td>
                                <td class="px-3 py-2 text-center text-gray-700">{{ $row->due_date ? \Carbon\Carbon::parse($row->due_date)->format('n/j/Y') : '—' }}</td>
                                <td class="px-3 py-2 text-center text-gray-700">{{ $today->format('n/j/Y') }}</td>
                                <td class="px-3 py-2 text-center text-gray-800 font-medium">{{ $row->dr_no ?? '—' }}</td>
                                <td class="px-3 py-2 text-center text-gray-800 font-medium">{{ $row->invoice_no ?? '—' }}</td>
                                <td class="px-3 py-2 text-center {{ $row->days_outstanding > 0 ? 'text-red-600 font-semibold' : 'text-green-700' }}">{{ $row->days_outstanding }}</td>
                                <td class="px-3 py-2 text-right text-gray-800">{{ $row->current > 0 ? number_format($row->current, 2) : '' }}</td>
            ...
            <div class="px-8 py-6 border-t border-gray-100">
                <div class="text-center space-y-0.5 mb-6">
                    ...
                    <p class="text-xs font-bold text-gray-700">Meatplus Trading Corporation</p>
                ...
                <div class="grid grid-cols-2 gap-8 mt-4">
                    <div>
                        <p class="text-xs font-bold text-gray-600 mb-4">Prepared by:</p>
                        <p class="text-sm text-gray-800 italic border-t border-gray-300 pt-1">{{ auth()->user()->name ?? 'Treasury' }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-xs font-bold text-gray-600 mb-4">Noted By:</p>
                        <p class="text-sm text-gray-800 italic border-t border-gray-300 pt-1">Fritz Daniel Martinez</p>
                    </div>
                </div>
            </div>
        </div>
