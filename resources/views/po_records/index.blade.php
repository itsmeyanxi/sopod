        ...
        <div class="flex flex-wrap gap-2 mb-6 border-b border-gray-700 pb-4">
            ...
            <a href="{{ route('po_records.index', ['type' => 'purchase_requests', 'status' => $status, 'from' => $from, 'to' => $to]) }}"
               class="px-4 py-2 rounded-t text-sm font-semibold transition
                {{ $type === 'purchase_requests' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-700' }}">
                Purchase Requests
            </a>
            @endif
            ...
            <a href="{{ route('po_records.index', ['type' => 'purchase_orders', 'status' => $status, 'from' => $from, 'to' => $to]) }}"
               class="px-4 py-2 rounded-t text-sm font-semibold transition
                {{ $type === 'purchase_orders' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-700' }}">
                Purchase Orders
            </a>
            @endif
            ...
            <a href="{{ route('po_records.index', ['type' => 'rfp', 'status' => $status, 'from' => $from, 'to' => $to]) }}"
               class="px-4 py-2 rounded-t text-sm font-semibold transition
                {{ $type === 'rfp' ? 'bg-orange-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-700' }}">
                Request for Payment
            </a>
            @endif
            ...
            <a href="{{ route('po_records.index', ['type' => 'apv', 'status' => $status, 'from' => $from, 'to' => $to]) }}"
               class="px-4 py-2 rounded-t text-sm font-semibold transition
                {{ $type === 'apv' ? 'bg-teal-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-700' }}">
                AP Voucher
            </a>
            @endif
            ...
            <a href="{{ route('po_records.index', ['type' => 'check_vouchers', 'status' => $status, 'from' => $from, 'to' => $to]) }}"
               class="px-4 py-2 rounded-t text-sm font-semibold transition
                {{ $type === 'check_vouchers' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-700' }}">
                Check Vouchers
            </a>
            @endif
        </div>
                ...
                <thead>
                    <tr class="bg-gray-700">
                        @if($type === 'purchase_requests')
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">PR No</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Company</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Requisitioner</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Supplier</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Date of Request</th>
                        ...
                        @elseif($type === 'purchase_orders')
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">PO No</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">PR No</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Company</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Supplier</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Order Date</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-center text-gray-400">Status</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Created By</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-center text-gray-400">Actions</th>
                        @elseif($type === 'rfp')
                            ...
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">PO No</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Company</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Payee</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-right text-gray-300">Amount</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Date</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-center text-gray-400">Status</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Created By</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-center text-gray-400">Actions</th>
                        @elseif($type === 'apv')
                            ...
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">RFP No</th>
                            <th class="px-4 py-2 border-b border-gray-700 text-left text-gray-300">Vendor</th>
                ...
                <tbody>
                    @forelse($records as $record)
                        <tr class="hover:bg-gray-700 transition">
                            @if($type === 'purchase_requests')
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->pr_no }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->company }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->requisitioner }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->supplier->supplier_name ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->date_of_request }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-center">
                                    @include('po_records._status_badge', ['status' => $record->status])
                                </td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->creator->name ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-center">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        @else
                                            <span class="bg-gray-700 text-gray-300 px-3 py-1 rounded text-xs cursor-not-allowed" title="Must be approved to print"><i class="fas fa-print"></i> Print</span>
                                        @endif
                                    </div>
                                </td>
                            @elseif($type === 'purchase_orders')
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->po_no }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->pr_no ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->company }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->supplier ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->order_date->format('M d, Y') }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-center">
                                    @include('po_records._status_badge', ['status' => $record->status])
                                </td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->creator->name ?? 'N/A' }}</td>
                            ...
                            @elseif($type === 'rfp')
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->rfp_no }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->purchaseOrder->po_no ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->company }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->payee }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-right text-gray-300">{{ number_format($record->amount, 2) }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->date }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-center">
                                    @include('po_records._status_badge', ['status' => $record->status])
                                </td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->creator->name ?? 'N/A' }}</td>
                            ...
                            @elseif($type === 'apv')
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->apv_no }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->requestForPayment->rfp_no ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->vendor_name }}</td>
                                ...
                                <td class="px-4 py-2 border-b border-gray-700 text-right text-gray-300">{{ $record->currency }} {{ number_format($record->grand_total, 2) }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-center">
                                    @include('po_records._status_badge', ['status' => $record->status])
                                </td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->creator->name ?? 'N/A' }}</td>
                            ...
                            @elseif($type === 'check_vouchers')
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->cv_no }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">
                                    @if($record->apv_no)
                                ...
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->supplier_name }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->bank ?? 'N/A' }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-right text-gray-300">{{ number_format($record->check_amount, 2) }}</td>
                                <td class="px-4 py-2 border-b border-gray-700 text-center">
                                    @include('po_records._status_badge', ['status' => $record->status])
                                </td>
                                <td class="px-4 py-2 border-b border-gray-700 text-gray-300">{{ $record->creator->name ?? 'N/A' }}</td>
