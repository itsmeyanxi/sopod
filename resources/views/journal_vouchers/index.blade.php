...
<div class="container mx-auto">
    <div class="bg-white text-gray-800 rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
            <h1 class="text-2xl font-bold text-gray-800">JOURNAL VOUCHERS</h1>
        ...
        <!-- Filters -->
        <form method="GET" action="{{ route('journal_vouchers.index') }}" class="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-6 gap-3">
                <div>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search JV#, description..." class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm">
                </div>
                <div>
                    <select name="type" class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm">
                ...
                <div>
                    <input type="date" name="date_from" value="{{ request('date_from') }}" class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm" placeholder="From">
                </div>
                <div>
                    <input type="date" name="date_to" value="{{ request('date_to') }}" class="w-full bg-white border border-gray-300 rounded px-3 py-2 text-sm" placeholder="To">
                </div>
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('journal_vouchers.index') }}" class="bg-gray-200 text-gray-700 px-3 py-2 rounded text-sm hover:bg-gray-300"><i class="fas fa-times"></i></a>
                </div>
            </div>
        </form>
            ...
            <table class="w-full text-sm border-collapse">
                <thead class="bg-gray-50 text-gray-500 uppercase text-xs">
                    <tr>
                        ...
                        <th class="border border-gray-200 px-3 py-3 text-center">Date</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">Type</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">Description</th>
                        <th class="border border-gray-200 px-3 py-3 text-left">Reference</th>
                        <th class="border border-gray-200 px-3 py-3 text-right">Debit</th>
                        <th class="border border-gray-200 px-3 py-3 text-right">Credit</th>
                        <th class="border border-gray-200 px-3 py-3 text-center">Lines</th>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-gray-50 border-b border-gray-100">
                        ...
                        <td class="border border-gray-200 px-3 py-2 text-center text-xs">{{ $jv->jv_date->format('M d, Y') }}</td>
                        <td class="border border-gray-200 px-3 py-2">
                            <span class="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs font-semibold">{{ $jv->formatted_type }}</span>
                        </td>
                        <td class="border border-gray-200 px-3 py-2 text-gray-700 max-w-xs truncate" title="{{ $jv->description }}">{{ Str::limit($jv->description, 40) }}</td>
                        <td class="border border-gray-200 px-3 py-2 text-gray-500 text-xs">{{ $jv->reference_no ?? '—' }}</td>
                        <td class="border border-gray-200 px-3 py-2 text-right font-semibold">{{ number_format($jv->total_debit, 2) }}</td>
                        <td class="border border-gray-200 px-3 py-2 text-right font-semibold">{{ number_format($jv->total_credit, 2) }}</td>
                        <td class="border border-gray-200 px-3 py-2 text-center text-xs">{{ $jv->lines_count }}</td>
