            ...
            <div class="flex items-center gap-4">
                <div class="text-right">
                    ...
                    @if($purchaseRequest->status === 'approved')
                        <a href="{{ route('purchase_requests.go_to_po', $purchaseRequest->id) }}" class="ml-2 px-4 py-1 bg-gray-50 border border-gray-200 text-blue-600 rounded hover:underline font-semibold" title="Click to go to PO">{{ $purchaseRequest->pr_no }}</a>
                    @else
                ...
                <table class="w-full border-collapse border border-gray-200">
                    <thead class="bg-gray-100 text-gray-500 uppercase text-xs">
                        <tr>
                            ...
                            <th class="border border-gray-200 px-4 py-3">REMARKS/SPECIFICATIONS</th>
                    ...
                    <tbody class="text-gray-500 divide-y divide-gray-700">
                        @foreach($purchaseRequest->items as $item)
                            <tr class="hover:bg-gray-100/40">
                                ...
                                <td class="border border-gray-200 px-4 py-3">{{ $item->date_needed ? \Carbon\Carbon::parse($item->date_needed)->format('M d, Y') : 'N/A' }}</td>
                                ...
                                <td class="border border-gray-200 px-4 py-3">{{ $item->supplier_name ?? 'N/A' }}</td>
                                <td class="border border-gray-200 px-4 py-3 text-right">{{ $item->unit_price ? '₱' . number_format($item->unit_price, 2) : 'N/A' }}</td>
                                <td class="border border-gray-200 px-4 py-3 text-right">{{ $item->amount ? '₱' . number_format($item->amount, 2) : 'N/A' }}</td>
                                <td class="border border-gray-200 px-4 py-3">{{ $item->remarks ?? 'N/A' }}</td>
                                <td class="border border-gray-200 px-4 py-3">{{ $item->note ?? '' }}</td>
                            </tr>
                        @endforeach
                        @if($purchaseRequest->items->count() > 0)
                            <tr class="font-semibold bg-gray-100">
                                <td colspan="8" class="border border-gray-200 px-4 py-3 text-right">TOTAL:</td>
                                <td class="border border-gray-200 px-4 py-3 text-right">₱{{ number_format($purchaseRequest->items->sum('amount'), 2) }}</td>
...
<div id="approveManagementModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-96">
        <h3 class="text-xl font-bold text-gray-800 mb-4">Approve as {{ str_contains(strtolower($purchaseRequest->company ?? ''), 'magalang') ? 'CFO' : 'Management' }}</h3>
...
<div id="rejectModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-96">
        <h3 class="text-xl font-bold text-gray-800 mb-4">Reject Purchase Request</h3>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg p-6 w-96">
        <h3 class="text-xl font-bold text-gray-800 mb-4">Delete Purchase Request</h3>
