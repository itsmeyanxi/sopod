    ...
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
                ...
                <table class="w-full border-collapse border border-gray-700">
                    ...
                    <tfoot>
                        <tr class="bg-gray-700">
                            ...
                            <td class="border border-gray-700 px-4 py-3 text-right font-bold text-white text-lg">&#8369;{{ number_format($liquidation->total_amount_spent, 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
                    ...
                    <tbody>
                        <tr>
                            <td class="border border-gray-700 px-4 py-8 text-center align-bottom">
                                <span class="text-white font-semibold text-sm">{{ $liquidation->submitted_by ?? $liquidation->creator->name ?? '' }}</span>
                            </td>
                            <td class="border border-gray-700 px-4 py-8 text-center align-bottom">
        ...
        @if($liquidation->status === 'pending' && $liquidation->approval_stage !== 'rejected')
            <div class="flex gap-3 mb-4 mt-6">
                @if($liquidation->approval_stage === 'pending_dh' && auth()->user()->hasRole(['Admin','IT','Department_Head','Procurement_Approver']))
                ...
                @if($liquidation->approval_stage === 'pending_executive' && auth()->user()->hasRole(['Admin','IT','President','Vice_President','CFO']))
...
<div id="approveDHModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg p-6 w-96">
        <h3 class="text-xl font-bold text-white mb-4">Approve as Immediate Superior</h3>
...
<div id="approveExecutiveModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg p-6 w-96">
        <h3 class="text-xl font-bold text-white mb-4">Approve as Executive</h3>
...
<div id="rejectModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg p-6 w-96">
        <h3 class="text-xl font-bold text-white mb-4">Reject Liquidation Form</h3>
