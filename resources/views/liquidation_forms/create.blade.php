...
@section('title', 'Create Liquidation Form')
...
<div class="container mx-auto">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
        ...
        <div class="mb-6 bg-gray-900 border border-gray-700 rounded p-4">
            ...
            <p class="text-gray-300 text-sm mb-3">Search by CAR Number, Payee Name, or Department to link a Cash Advance Request</p>
            <div class="relative">
                <input
                    type="text"
                    id="carSearchInput"
                ...
                @if($selectedCAR)
                    <div class="p-3 bg-green-50 border border-green-200 rounded text-green-700 flex justify-between items-center">
                        <span>
                            ...
                            <span class="text-gray-300 ml-2">(CAR Amount: &#8369;{{ number_format($selectedCAR->amount, 2) }})</span>
                        </span>
                        <button type="button" id="unlinkCAR" class="text-red-700 hover:text-red-700 text-sm"><i class="fas fa-times"></i></button>
                    </div>
                @else
                    <div class="p-3 bg-gray-800 border border-gray-700 rounded text-gray-300">
        ...
        <form action="{{ route('liquidation_forms.store') }}" method="POST" id="liquidationForm" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    <label class="block font-semibold text-gray-300 mb-2">NAME: <span class="text-red-700">*</span></label>
            ...
            <div class="flex justify-end gap-4">
                <a href="{{ route('liquidation_forms.index') }}" class="bg-gray-700 text-white px-6 py-2 rounded hover:bg-gray-700 transition">
                    Cancel
                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (carSearchInput) {
        carSearchInput.addEventListener('input', function() {
            ...
            carSearchResults.innerHTML = '<div class="p-3 text-gray-300 text-center"><i class="fas fa-spinner fa-spin mr-2"></i>Searching...</div>';
                    ...
                    .then(cars => {
                        ...
                        carSearchResults.querySelectorAll('.car-result-item').forEach(item => {
                            item.addEventListener('click', function() {
                                ...
                                // Fill name and department from CAR
                                ...
                                document.getElementById('linkedCARDisplay').innerHTML = `
                                    <div class="p-3 bg-green-50 border border-green-200 rounded text-green-700 flex justify-between items-center">
                                        <span>
                                            ...
                                            — ${payee}
                                            <span class="text-gray-300 ml-2">(CAR Amount: &#8369;${parseFloat(amount).toLocaleString('en-US', {minimumFractionDigits: 2})})</span>
                                        </span>
                                        <button type="button" id="unlinkCAR" class="text-red-700 hover:text-red-700 text-sm"><i class="fas fa-times"></i></button>
                                    </div>
                                `;
    ...
    function attachUnlinkHandler() {
        ...
        if (unlinkBtn) {
            unlinkBtn.addEventListener('click', function() {
                ...
                document.getElementById('liqName').value = '';
                document.getElementById('liqDepartment').value = '';
            });
        }
    }
    attachUnlinkHandler();
