...
<div class="container mx-auto px-4 py-6">
    <div class="bg-gray-800 text-white rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-blue-900 rounded-lg p-4 border-l-4 border-blue-500">
                <p class="text-gray-300 text-sm">Total Adjustments</p>
                <p class="text-3xl font-bold text-blue-400">{{ $summary['total_count'] }}</p>
            </div>
            <div class="bg-green-900 rounded-lg p-4 border-l-4 border-green-500">
                <p class="text-gray-300 text-sm">Total Credit (Decrease)</p>
                <p class="text-2xl font-bold text-green-400">₱{{ number_format($summary['credit_total'], 2) }}</p>
            </div>
            <div class="bg-red-900 rounded-lg p-4 border-l-4 border-red-500">
                <p class="text-gray-300 text-sm">Total Debit (Increase)</p>
                <p class="text-2xl font-bold text-red-400">₱{{ number_format($summary['debit_total'], 2) }}</p>
            </div>
            <div class="bg-purple-900 rounded-lg p-4 border-l-4 border-purple-500">
                <p class="text-gray-300 text-sm">Net Impact</p>
                <p class="text-2xl font-bold {{ $summary['net_total'] < 0 ? 'text-red-400' : 'text-green-400' }}">
        ...
        {{-- AR Aging Summary --}}
        @if($arAging->count() > 0)
        <div class="mb-6 bg-blue-900/20 border border-blue-700 rounded-lg p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div class="bg-gray-700/50 p-3 rounded">
                    <div class="flex justify-between items-start mb-1">
                        ...
                        <span class="text-yellow-400 font-bold">₱{{ number_format($record->net_ar_balance ?? 0, 2) }}</span>
                    </div>
                    <div class="text-gray-400 text-xs">
                        <div>Date: {{ $record->invoice_date?->format('M d, Y') ?? 'N/A' }}</div>
                        <div>Status: <span class="text-blue-300">{{ ucfirst($record->status ?? 'Outstanding') }}</span></div>
                    </div>
                </div>
                @endforeach
            </div>
                ...
                <table class="w-full border-collapse border border-gray-700 text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-700 hover:bg-gray-700/50">
                            ...
                            <td class="border border-gray-700 px-4 py-3 text-right font-semibold {{ $adj->is_decrease ? 'text-red-400' : 'text-green-400' }}">
