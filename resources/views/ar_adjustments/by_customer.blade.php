    ...
    <div class="bg-white text-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
            ...
            <div class="flex gap-3">
                <a href="{{ route('ar_adjustments.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded transition flex items-center gap-2">
        ...
        <div class="mb-6 bg-blue-50 border border-blue-700 rounded-lg p-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div class="bg-gray-100/50 p-3 rounded">
                    <div class="flex justify-between items-start mb-1">
                        <span class="font-semibold text-gray-800">{{ $record->invoice_no ?? 'N/A' }}</span>
                ...
                <table class="w-full border-collapse border border-gray-200 text-sm">
                    <thead class="bg-gray-50 text-gray-500">
                        <tr>
                            ...
                            <th class="border border-gray-200 px-4 py-3 text-left">DR No</th>
                            <th class="border border-gray-200 px-4 py-3 text-left">Invoice No</th>
                            <th class="border border-gray-200 px-4 py-3 text-right">Amount</th>
                            <th class="border border-gray-200 px-4 py-3 text-left">GL Account</th>
                            <th class="border border-gray-200 px-4 py-3 text-left">Remarks</th>
                            <th class="border border-gray-200 px-4 py-3 text-center">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($adjustments as $adj)
                        <tr class="border-b border-gray-200 hover:bg-gray-100/50">
                            <td class="border border-gray-200 px-4 py-3">{{ $adj->transaction_date->format('M d, Y') }}</td>
                            ...
                            <td class="border border-gray-200 px-4 py-3 text-right font-semibold {{ $adj->is_decrease ? 'text-red-700' : 'text-green-700' }}">
                                {{ $adj->is_decrease ? '-' : '+' }}₱{{ number_format(abs($adj->amount), 2) }}
                            </td>
                            <td class="border border-gray-200 px-4 py-3 text-xs">{{ $adj->gl_account }}</td>
                            <td class="border border-gray-200 px-4 py-3 text-xs max-w-xs truncate" title="{{ $adj->remarks ?? '' }}">
                                {{ $adj->remarks ?? '—' }}
                            </td>
