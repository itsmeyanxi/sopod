        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('payments.edit', $payment->id) }}" class="px-3 py-1 rounded-full text-xs font-bold bg-white text-blue-700 hover:bg-blue-50 transition">
                        <i class="fas fa-edit mr-1"></i>{{ auth()->user()->canEditPayments() ? 'Edit' : 'Request Edit' }}
                    </a>
                    @endif
                    <span class="px-3 py-1 rounded-full text-xs font-bold
                        {{ ($payment->status ?? 'Posted') === 'Posted' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ $payment->customer_name ?? '—' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-gray-500">Customer Code</span>
                        <span class="text-sm text-gray-800">{{ $payment->customer_code ?? '—' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-gray-500">Branch</span>
                        <span class="text-sm text-gray-800">{{ $payment->branch ?? '—' }}</span>
                    </div>
                </div>
            </div>
            <div>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span class="text-sm font-semibold text-gray-800">{{ $payment->invoice_no ?? '—' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-gray-500">DR No</span>
                        <span class="text-sm text-gray-800">{{ $payment->dr_no ?? '—' }}</span>
                    </div>
                    @if($payment->invoice_outstanding)
                    <div class="flex justify-between">
                        <span class="text-sm text-gray-500">Outstanding at Payment</span>
                        <span class="text-sm text-gray-800">₱{{ number_format((float)$payment->invoice_outstanding, 2) }}</span>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="p-6 border-b border-gray-100">
            ...
            <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-lg font-bold text-gray-800">₱{{ number_format((float)($payment->gross_amount ?? $payment->amount ?? 0), 2) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">EWT</span>
                ...
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Credit Applied</span>
                ...
                <div class="flex justify-between items-center">
                    <span class="text-sm font-bold text-gray-700">Net Amount (Check Amount)</span>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800">{{ $checkNumber }}</p>
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-800">{{ \Carbon\Carbon::parse($dueDate)->format('M d, Y') }}</p>
                    </div>
                    @endif
                    @if(!empty($glAccount))
                    <div>
                        <span class="text-gray-500">G/L Account</span>
                        <p class="font-semibold text-gray-800">{{ $glAccount }}</p>
                    </div>
                    @endif
                    @if(!empty($meansAmount))
                    <div>
                        <span class="text-gray-500">Check/Transfer Amount</span>
                        <p class="font-semibold text-gray-800">₱{{ number_format((float)$meansAmount, 2) }}</p>
                    </div>
                    @endif
                ...
                <div class="mt-3 pt-3 border-t border-gray-200">
                    ...
                    <p class="text-sm text-gray-800">{{ $payment->payment_notes }}</p>
                </div>
                @endif
            </div>
        </div>
